...
#Who owns the tests ?
we want to have lots of these tests: if it is a significant system we will have 10 of thousands of these tests
...
=> you will overtest (no problem)

Developers own the Acceptance Tests
they own the responsibility to keep them working
because developers will break the tests as they add code, so it is their responsibility to fix the tests.
don't have a separate QA team write the tests, as developers add code, the tests break, and the qa team will not be able to catch up => ending in tests that are broken all the time
...
* Tests ANY change
* Efficient: we want to run 1000, 10 of 1000 of these tests
...
* isolation works on multiple levels:
   * isolating the SUT
   * isolating test cases from each other => run in parallel
   * isolating test cases from themselves (temporal isolation) => repeatable results when running a test case multiple times
...
it is fairly common in large enterprises to have an integration test environment where you glue all these things together. it is a requirement that you test it in these circumstances.
...
=> this assumes that you have global knowledge of all the systems and in depth understanding: this is not going to happen
...
Test Cases -> External System B -> Verifiable Output
...
=> you have the opportunity to plugin another implementation for the communication => TestStub Simulating the External System
...
TestStub makes part of the Test Infrastructure 
...
so you don't want that the tests takes days to run
...
#Scaling-Up
if you have don all the previous, this becomes much more easy
...
* Don' assume Nasty Automated Testing Products(tm) will do what you need. Be very sceptical about them. Start with YOUR strategy and evaluate tools against that.

* Don't have separate QA team. Quality is down to everyone - Developers own Acceptance Testing
...
#Questions
open source product: LMAX Simple DSL
does the aliasing
...
About parallelising tests and not starting and initing the app
