...
resource "ovh_domain_zone_record" "io_thinkinglabs_records" {
  count     = length(local.io_zone_records)
  zone      = local.io_zone
  subdomain = local.io_zone_records[count.index].subdomain
  fieldtype = local.io_zone_records[count.index].fieldtype
  ttl       = 0
  target    = local.io_zone_records[count.index].target
}
